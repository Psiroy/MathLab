...
void logicalize(expn en, varlist* pvl) {
	...
	if (s == 2) {
		...
		if (en->pre != NULL && en->pre->func[0] == '=') {
			en->ep = insertdigt(en->ep, '[', 'p');
			insertdigt(topnum(en->ep), ']', 'n');
		}
		return;
	}
	ep = en->ep;
...
int isoperator(digt* p) {
	if (p == NULL) return 0;
	if (p->d == ',' || p->d == '!' || p->d == ':' ||p->d == '@'||p->d == '%'||p->d == '^'||p->d == '&'||p->d == '*'||p->d == '-'||p->d == '+'||p->d == '='||p->d == '|'||p->d == '<'||p->d == '>'||p->d == '/'||p->d == '?') return 1;
	return 0;
}
int charcheckforexpr(expr ep) {
	digt* p = bottomnum(ep);
...
int isdoubleoperator(digt* p) {
	if (p == NULL) return 0;
	if (p->d == ',' || p->d == '@' || p->d == '%' || p->d == '^' || p->d == '&' || p->d == '*' || p->d == '-' || p->d == '+' || p->d == '=' || p->d == '|' || p->d == '<' || p->d == '>' || p->d == '/' || p->d == '?') return 1;
	return 0;
}
expr replace(expr ep, expr before, expr after) {
	digt* p = ep, *q, *r, *s;
...
void decoration(expn en) {
	...
	if (en->pre != NULL) {
		...
		//else if (en->type == 3 || en->type == 5) {
			...
			if (r == s && (((r == 3 || r == 12 || r == 15) && u == 1) || (u == 0 && r != 15 && r != 12 && r != 3))) t = 0;
